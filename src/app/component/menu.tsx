  ...
  return (
    <Section className="w-fit flex md:hidden  overflow-scroll  " >
     ...
     <div className={`absolute top-0 left-0 w-full h-full overflow-y-auto bg-transparent  ${open?'translate-x-0 transition-all duration-300':'-translate-x-[320px] transition-all duration-300'}`}>
        <div className="bg-logo flex flex-col justify-center gap-10  w-[320px] h-full">
     {link.map((links)=>{
                return(
                   <Link scroll={false}  onClick={()=>setOpen(false)} className="border-2 w-32 mx-auto rounded-lg text-center  font-semibold border-green-600 hover:bg-green-600 px-2 py-2" 
